...
| `date` | 필수 | 파일명 날짜와 일치, 한국 시간대 `+0900` 명시. **현재 시각보다 이전으로 설정** |
| `categories` | 권장 | 1~2개, 대분류 중심 |
| `tags` | 권장 | 3~5개, 소문자 |

주의사항:
- ⚠️ **`date`가 현재 시각보다 미래면 Jekyll이 글을 빌드에서 제외한다.** 게시 시점에서 시간을 현재보다 이전으로 설정해야 한다. (예: 게시 시각이 2026-09-07 14:30이면, `date: 2026-09-07 10:00:00 +0900` 처럼 더 이른 시간으로)
...
- [ ] **date가 현재 시각보다 이전인가?** (미래 시간이면 Jekyll이 빌드 제외)
